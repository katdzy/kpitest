...
@section('title', 'Edit KPI - ' . $kpi->measure_code)
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-3">
        <a href="{{ route('kpis.show', $kpi->id) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:text-indigo-600 hover:border-indigo-300 shadow-sm transition-all">
            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Edit KPI Version</h1>
            <p class="text-xs text-slate-500 mt-0.5">
                Updating KPI code <strong class="text-slate-800 font-mono">{{ $kpi->measure_code }}</strong> for active year <strong class="text-slate-800 font-mono">{{ $kpi->year_range }}</strong>.
            </p>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('kpis.update', $kpi->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="rounded-2xl bg-white border border-slate-200/60 shadow-sm p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           class="block w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-colors bg-slate-50 cursor-not-allowed text-slate-500"
                           readonly
                    >
                    <span class="text-[10px] text-slate-400 mt-1 block">Primary code is immutable for this version.</span>
                </div>
                ...
                <div>
                    ...
                    <input type="text" 
                           name="year_range" 
                           id="year_range" 
                           value="{{ old('year_range', $kpi->year_range) }}"
                           placeholder="e.g. 2025-2026" 
                           class="block w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-colors bg-slate-50 cursor-not-allowed text-slate-500"
                           readonly
                    >
                    <span class="text-[10px] text-slate-400 mt-1 block">To edit year, clone this KPI to a new version.</span>
                </div>
                ...
                <div>
                    ...
                    >
                        @foreach (\App\Models\Kpi::STATUSES as $s)
                            <option value="{{ $s }}" {{ old('status', $kpi->status) === $s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
            ...
            <!-- Description -->
            <div>
                <label for="description" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Description</label>
        ...
        <div class="rounded-2xl bg-white border border-slate-200/60 shadow-sm p-6 space-y-5">
            ...
            <!-- Target Rationale -->
            <div>
                <label for="target_rationale" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Target Rationale</label>
        ...
        <div class="rounded-2xl bg-white border border-slate-200/60 shadow-sm p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                <!-- Data Provider -->
                <div>
                    <label for="data_provider" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Data Provider</label>
                    <input type="text" 
                           name="data_provider" 
                           id="data_provider" 
                ...
                <div>
                    <label for="collection_frequency" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1 font-sans">Collection Frequency</label>
                    <input type="text" 
        ...
        <div class="flex items-center justify-end space-x-3.5 pt-4">
            <a href="{{ route('kpis.show', $kpi->id) }}" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl transition-all">
                Cancel
            </a>
