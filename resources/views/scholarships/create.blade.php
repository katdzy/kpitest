...
@section('title', 'Add Scholarship / Grant')
@section('page_title', 'Scholarships & Grants — Add New')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <form action="{{ route('scholarships.store') }}" method="POST" class="space-y-5">
        @csrf
        ...
        <div class="rounded-2xl bg-white border border-slate-200/60 shadow-sm p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div>
                    ...
                    <select id="type" name="type" required class="block w-full rounded-xl border border-slate-200 py-2.5 px-3 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 bg-white @error('type') border-red-300 @enderror">
                ...
                <div>
                    ...
                    <select id="beneficiary_type" name="beneficiary_type" required class="block w-full rounded-xl border border-slate-200 py-2.5 px-3 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 bg-white @error('beneficiary_type') border-red-300 @enderror">
                        <option value="">Select</option>
                        @foreach(['Student','Faculty','Staff','Both'] as $bt)
                            <option value="{{ $bt }}" {{ old('beneficiary_type') === $bt ? 'selected' : '' }}>{{ $bt }}</option>
                        @endforeach
                    </select>
                    @error('beneficiary_type')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <select id="academic_year" name="academic_year" required class="block w-full rounded-xl border border-slate-200 py-2.5 px-3 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 bg-white @error('academic_year') border-red-300 @enderror">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" id="administering_unit" name="administering_unit" value="{{ old('administering_unit') }}"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="block w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">{{ old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="rounded-2xl bg-white border border-slate-200/60 shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div>
                    ...
                    <input type="text" id="funding_source" name="funding_source" value="{{ old('funding_source') }}" placeholder="e.g. CHED, DOST, Private"
                           class="block w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select id="currency" name="currency" class="block w-full rounded-xl border border-slate-200 py-2.5 px-3 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 bg-white">
                        @foreach(['PHP','USD','EUR'] as $c)
                            <option value="{{ $c }}" {{ old('currency','PHP') === $c ? 'selected' : '' }}>{{ $c }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div>
                    ...
                    <input type="number" min="0" id="beneficiaries_count" name="beneficiaries_count" value="{{ old('beneficiaries_count') }}"
                           class="block w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                </div>
                <div>
            ...
            <div>
                ...
                <input type="text" id="selection_criteria" name="selection_criteria" value="{{ old('selection_criteria') }}"
                       class="block w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
            </div>
        </div>
        ...
        <div class="rounded-2xl bg-white border border-slate-200/60 shadow-sm p-6 space-y-4">
            ...
            <div>
                ...
                <textarea id="outcomes" name="outcomes" rows="3" class="block w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">{{ old('outcomes') }}</textarea>
            </div>
            <div>
                <label for="remarks" class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Remarks</label>
                <textarea id="remarks" name="remarks" rows="2" class="block w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">{{ old('remarks') }}</textarea>
            </div>
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label for="item_author" class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Item Author</label>
                    <input type="text" id="item_author" name="item_author" value="{{ old('item_author') }}"
                           class="block w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                </div>
                <div>
        ...
        <div class="flex items-center justify-end space-x-3">
            <a href="{{ route('scholarships.index') }}" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-all">Cancel</a>
            <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-amber-500 hover:bg-amber-600 rounded-xl shadow-sm transition-all">Save Record</button>
        </div>
    </form>
</div>
@endsection
