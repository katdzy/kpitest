...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-3">
        <a href="{{ route('departments.show', $department->id) }}" class="inline-flex items-center text-sm text-slate-500 hover:text-indigo-600 font-medium transition-colors">
    ...
    <div class="rounded-2xl bg-white border border-slate-200/60 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/60">
            <h2 class="text-lg font-bold text-slate-900">Edit Department</h2>
            <p class="text-sm text-slate-500 mt-0.5">Updating: <span class="font-mono font-bold text-slate-700">{{ $department->code }}</span> — {{ $department->name }}</p>
        </div>

        <form action="{{ route('departments.update', $department->id) }}" method="POST" class="px-6 py-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="college" id="college" value="{{ old('college', $department->college) }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="block w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('description', $department->description) }}</textarea>
            </div>
            ...
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-100">
                <a href="{{ route('departments.show', $department->id) }}" class="px-4 py-2.5 text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all">
                    Cancel
                </a>
